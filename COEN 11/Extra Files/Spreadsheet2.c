...
char insert(void){
	...
	if(counter == 10) printf("\nError: List Full\n");
		...
		if(slot != -2){
			...
			for(k = 0; k<counter;k++) q++;

			for(k = counter; k > slot; k--, q--){
				strcpy(listProfiles[k].firstName,  listProfiles[k-1].firstName);
				strcpy(listProfiles[k].lastName, listProfiles[k-1].lastName);
				listProfiles[k].age = listProfiles[k-1].age;
			}

		strcpy(q->firstName, userFirst);	
                strcpy(q->lastName, userLast);
		q->age = userAge;
		counter++;
